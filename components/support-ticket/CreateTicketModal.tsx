...
export default function CreateTicketModal({
  ...
  const { isVerifying, isVerified, verifiedTicketData, resetVerification, setIsVerified } = useVerificationStore();
    ...
    if (isOpen) {
      const fetchData = async () => {
        try {
          ...
          setError(locales.tickets?.form?.errors?.failed_to_load_departments || "Failed to load departments. Please try again later.");
          setCategories([]);
        } finally {
          setLoading(false);
        }
  ...
  }, [selectedCategoryId, isOpen, setValue]);
  ...
  const onSubmit = async (data: TicketFormData) => {
    try {
      ...
      setSubmitError(
        ...
        locales.tickets?.form?.errors?.failed_to_create_ticket || "Failed to create ticket. Please try again."
      );
    } finally {
            ...
            <Dialog.Panel className="w-full max-w-2xl bg-card rounded-2xl shadow-2xl border border-border max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between p-6 border-b border-border">
                <Dialog.Title className="text-2xl font-bold text-foreground">
                  {isVerifying ? (locales.tickets?.modal?.verify_title || "Verify Your Ticket") : (locales.tickets?.modal?.create_title || "Create Support Ticket")}
                </Dialog.Title>
                <button
                  onClick={handleClose}
                      ...
                      {loading ? (
                        <div className="flex items-center justify-center py-8">
                          ...
                          <span className="ml-2 text-muted-foreground">
                            {locales.tickets?.modal?.loading_departments || "Loading departments..."}
                          </span>
                        </div>
                      ) : (
                        <>
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.main_topic_label || "Main Topic"} <span className="text-destructive">*</span>
                            </label>
                            <select
                              ...
                              disabled={loading}
                            >
                              <option value="">{locales.tickets?.modal?.main_topic_placeholder || "Select a main topic"}</option>
                              {categories.map((cat) => (
                          ...
                          {availableSubDepts.length > 0 && (
                            <div>
                              <label className="block text-sm font-medium text-foreground mb-2">
                                {locales.tickets?.modal?.specific_issue_label || "Specific Issue"}
                              </label>
                              <select
                              ...
                              >
                                <option value="">{locales.tickets?.modal?.specific_issue_placeholder || "Select a specific issue (optional)"}</option>
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.full_name_label || "Full Name"} <span className="text-destructive">*</span>
                            </label>
                            <input
                              type="text"
                              {...register("guestName")}
                              ...
                              placeholder={locales.tickets?.modal?.full_name_placeholder || "Enter your full name"}
                            />
                            {errors.guestName && (
                              <p className="mt-1 text-sm text-destructive">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.email_label || "Email Address"} <span className="text-destructive">*</span>
                            </label>
                            <input
                              type="email"
                              {...register("guestEmail")}
                              ...
                              placeholder={locales.tickets?.modal?.email_placeholder || "Enter your email address"}
                            />
                            {errors.guestEmail && (
                              <p className="mt-1 text-sm text-destructive">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.phone_label || "Phone Number"} <span className="text-destructive">*</span>
                            </label>
                            <input
                              type="tel"
                              {...register("guestPhone")}
                              ...
                              placeholder={locales.tickets?.modal?.phone_placeholder || "Enter your phone number"}
                            />
                            {errors.guestPhone && (
                              <p className="mt-1 text-sm text-destructive">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.subject_label || "Subject"} <span className="text-destructive">*</span>
                            </label>
                            <input
                              type="text"
                              {...register("subject")}
                              ...
                              placeholder={locales.tickets?.modal?.subject_placeholder || "Enter ticket subject"}
                            />
                            {errors.subject && (
                              <p className="mt-1 text-sm text-destructive">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.description_label || "Description"} <span className="text-destructive">*</span>
                            </label>
                            <textarea
                              rows={4}
                              ...
                              placeholder={locales.tickets?.modal?.description_placeholder || "Describe your issue in detail"}
                            />
                            {errors.description && (
                              <p className="mt-1 text-sm text-destructive">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-foreground mb-2">
                              {locales.tickets?.modal?.attachments_label || "Attachments"}
                              <span className="block text-xs text-muted-foreground font-normal mt-1">
                                {locales.tickets?.modal?.attachments_max_size || "Maximum 10MB per file"}
                              </span>
                            </label>
                            <input
                              type="file"
                            ...
                            >
                              ...
                              <span>{locales.tickets?.modal?.choose_files || "Choose Files"}</span>
                            </button>
                            {attachments.length > 0 && (
                              ...
                              >
                                <span className="text-sm text-foreground">
                                  {attachments.length} {locales.tickets?.modal?.files_selected || "file(s) selected"}
                                </span>
                                <button
                                  type="button"
                            ...
                            >
                              {isSubmitting ? (
                                <>
                                  ...
                                  {locales.tickets?.modal?.submitting || "Submitting..."}
                                </>
                              ) : (
                                locales.tickets?.modal?.submit || "Submit Ticket"
                              )}
                            </button>
                          </div>
                        </>
